...
{
    // Fix applied to suppress CS8618 warning using nullable reference types
...
        int expectedMin = 50;
        ...
        GameDifficulty difficulty = GameDifficulty.Easy; // Range = 10

        _mockRng!.Setup(rng => rng.GenerateMinBoundary()).Returns(expectedMin); // Typo fix

        // 1. Calculate the expected INCLUSIVE max value (e.g., 60)
        var intendedMaxValue = expectedMin + (int)difficulty;

        // 2. Mock the call using the required EXCLUSIVE max boundary (e.g., 61)
...
            // Assert the Range.MaxValue is the inclusive max (60)
...
        int correctGuess = 42;
        ...
        {
            GuessCount = 5 // Set the starting count for the test
        };
...
    // Uses corrected enum names (TooHigh, TooLow)
...
        GameState finishedState = new GameState(
      ...
      mysteryNumber: 5
    ) with
        {
            GuessCount = 3,
            IsGameWon = true,
            IsGameOver = true // Already finished
        };
